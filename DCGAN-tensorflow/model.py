          ...
          try:
            ...
            if config.dataset == 'mnist':
                ...
                save_images(samples, image_manifold_size(samples.shape[0]),
                        os.path.join(config.sample_dir, 'train_{:08d}.png'.format(counter)))
                ...
                # 使用绝对路径保存样本
                save_path = os.path.join(os.path.abspath(config.sample_dir), 
                                       'train_{:08d}.png'.format(counter))
                save_images(samples, image_manifold_size(samples.shape[0]), save_path)
                print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss))
          except Exception as e:
            print("Error saving sample: ", str(e))
            print("one pic error!...")
