...
flags.DEFINE_integer("ckpt_freq", 1000, "save checkpoint every this many iterations")
...
def main(_):
  ...
  with tf.Session(config=run_config) as sess:
    ...
    # 修改sample_dir的处理
    if FLAGS.sample_dir:
        FLAGS.sample_dir = os.path.expanduser(FLAGS.sample_dir)
        if not os.path.exists(FLAGS.sample_dir):
            os.makedirs(FLAGS.sample_dir)
    else:
        FLAGS.sample_dir = os.path.join(FLAGS.out_dir, 'samples')
        if not os.path.exists(FLAGS.sample_dir):
            os.makedirs(FLAGS.sample_dir)
